# ==========================================
# EEG → Dynamic Predictor (binary classification via optical flow)
# ==========================================
import os, random
import numpy as np
import torch
...
LOSS_TYPE        = "crossentropy"   # binary crossentropy (via CE with 2 outputs)
...
# NOTE: we reduce out_dim=2 for binary labels
MODEL_MAP = {
    "segments": lambda: models.glfnet(out_dim=2, emb_dim=emb_dim_segments, C=62, T=200),
    "DE":       lambda: models.glfnet_mlp(out_dim=2, emb_dim=emb_dim_DE, input_dim=62*5),
    "PSD":      lambda: models.glfnet_mlp(out_dim=2, emb_dim=emb_dim_PSD, input_dim=62*5),
}
...
class FusionNet(nn.Module):
    def __init__(self, encoders, num_classes=2):
        super().__init__()
...
# Labels (placeholder)
# ==========================================
# TODO: load All_video_optical_flow_score.npy
# Example:
# optical_flow = np.load("/content/drive/MyDrive/EEG2Video_data/raw/meta-info/All_video_optical_flow_score.npy")
# dynamic_labels = (optical_flow >= 1.799).astype(int)
# dynamic_labels should be shaped like (7,400) to align with EEG blocks
dynamic_labels = np.zeros((7,400), dtype=int)  # placeholder
...
for subname in sub_list:
    ...
    # Use dynamic_labels instead of All_label
    Top_1 = []
    for test_set_id in range(7):
        ...
        train_label = np.concatenate([dynamic_labels[i] for i in range(7) if i not in [test_set_id,val_set_id]])
        val_label   = dynamic_labels[val_set_id]
        test_label  = dynamic_labels[test_set_id]
        ...
        if multi:
            ...
            modelnet   = FusionNet(encoders, num_classes=2)
        else:
            ft = FEATURE_TYPES[0]
    ...
    print(f"{subname} finished dynamic predictor training.")

# '''
# Description: 
# Author: Zhou Tianyi
# Date: 2025-01-15 13:19:15
# LastEditTime: 2025-03-12 12:37:42
# LastEditors:  
# '''
# import decord
# decord.bridge.set_bridge('torch')

# from torch.utils.data import Dataset
# from einops import rearrange


# class TuneAVideoDataset(Dataset):
#     def __init__(
#             self,
#             video_path: str,
#             prompt: str,
#             width: int = 512,
#             height: int = 288, # UPDATED RESOLUTION
#             n_sample_frames: int = 8,
#             sample_start_idx: int = 0,
#             sample_frame_rate: int = 1,
#     ):
#         self.video_path = video_path
#         self.prompt = prompt
#         self.prompt_ids = None

#         self.width = width
#         self.height = height
#         self.n_sample_frames = n_sample_frames
#         self.sample_start_idx = sample_start_idx
#         self.sample_frame_rate = sample_frame_rate

#     def __len__(self):
#         return 1

#     def __getitem__(self, index):
#         # load and sample video frames
#         vr = decord.VideoReader(self.video_path, width=self.width, height=self.height)
#         sample_index = list(range(self.sample_start_idx, len(vr), self.sample_frame_rate))[:self.n_sample_frames]
#         video = vr.get_batch(sample_index)
#         video = rearrange(video, "f h w c -> f c h w")

#         example = {
#             "pixel_values": (video / 127.5 - 1.0),
#             "prompt_ids": self.prompt_ids
#         }

#         return example
# class TuneMultiVideoDataset(Dataset):
#     def __init__(
#             self,
#             video_path: list,
#             prompt: list,
#             width: int = 512,
#             height: int = 288, # UPDATED RESOLUTION
#             n_sample_frames: int = 24,
#             sample_start_idx: int = 0,
#             sample_frame_rate: int = 1,
#     ):
#         self.video_path = video_path
#         self.prompt = prompt
#         self.prompt_ids = None

#         self.width = width
#         self.height = height
#         self.n_sample_frames = n_sample_frames
#         self.sample_start_idx = sample_start_idx
#         self.sample_frame_rate = sample_frame_rate

#     def __len__(self):
#         return len(self.prompt)

#     def __getitem__(self, index):
#         # load and sample video frames
#         vr = decord.VideoReader(self.video_path[index], width=self.width, height=self.height)
#         sample_index = list(range(self.sample_start_idx, len(vr), self.sample_frame_rate))[:self.n_sample_frames]
#         video = vr.get_batch(sample_index)
#         video = rearrange(video, "f h w c -> f c h w")

#         example = {
#             "pixel_values": (video / 127.5 - 1.0),
#             "prompt_ids": self.prompt_ids[index]
#         }

#         return example

import decord
decord.bridge.set_bridge('torch')

import numpy as np
import torch
from torch.utils.data import Dataset
from einops import rearrange


class TuneAVideoDataset(Dataset):
    def __init__(
            self,
            video_path: str,
            prompt: str,
            use_latents: bool = True,  # NEW FLAG
            width: int = 512,
            height: int = 288,
            n_sample_frames: int = 24,
            sample_start_idx: int = 0,
            sample_frame_rate: int = 1,
    ):
        self.video_path = video_path
        self.prompt = prompt
        self.prompt_ids = None
        self.use_latents = use_latents

        self.width = width
        self.height = height
        self.n_sample_frames = n_sample_frames
        self.sample_start_idx = sample_start_idx
        self.sample_frame_rate = sample_frame_rate

    def __len__(self):
        return 1

    def __getitem__(self, index):
        if self.use_latents:
            # load precomputed latent npy file [f, c, h, w]
            arr = np.load(self.video_path)
            latents = rearrange(arr, "f c h w -> c f h w")  # -> [c, f, h, w]
            example = {
                "latents": torch.from_numpy(latents).float(),
                "prompt_ids": self.prompt_ids
            }
        else:
            # load and sample video frames
            vr = decord.VideoReader(self.video_path, width=self.width, height=self.height)
            sample_index = list(range(self.sample_start_idx, len(vr), self.sample_frame_rate))[:self.n_sample_frames]
            video = vr.get_batch(sample_index)
            video = rearrange(video, "f h w c -> f c h w")
            example = {
                "pixel_values": (video / 127.5 - 1.0),
                "prompt_ids": self.prompt_ids
            }
        return example


class TuneMultiVideoDataset(Dataset):
    def __init__(
            self,
            video_path: list,
            prompt: list,
            use_latents: bool = True,  # NEW FLAG
            width: int = 512,
            height: int = 288,
            n_sample_frames: int = 24,
            sample_start_idx: int = 0,
            sample_frame_rate: int = 1,
    ):
        self.video_path = video_path
        self.prompt = prompt
        self.prompt_ids = None
        self.use_latents = use_latents

        self.width = width
        self.height = height
        self.n_sample_frames = n_sample_frames
        self.sample_start_idx = sample_start_idx
        self.sample_frame_rate = sample_frame_rate

    def __len__(self):
        return len(self.prompt)

    def __getitem__(self, index):
        if self.use_latents:
            # load precomputed latent npy file [f, c, h, w]
            arr = np.load(self.video_path[index])
            latents = rearrange(arr, "f c h w -> c f h w")  # -> [c, f, h, w]
            example = {
                "latents": torch.from_numpy(latents).float(),
                "prompt_ids": self.prompt_ids[index]
            }
        else:
            # load and sample video frames
            vr = decord.VideoReader(self.video_path[index], width=self.width, height=self.height)
            sample_index = list(range(self.sample_start_idx, len(vr), self.sample_frame_rate))[:self.n_sample_frames]
            video = vr.get_batch(sample_index)
            video = rearrange(video, "f h w c -> f c h w")
            example = {
                "pixel_values": (video / 127.5 - 1.0),
                "prompt_ids": self.prompt_ids[index]
            }
        return example

...
class MyEEGNet_embedding(nn.Module):
    ...
    def build_embedding(self, x_shape, device):
        with torch.no_grad():
            x = torch.zeros(x_shape, device=device)
            x = self.block_1(x)
            x = self.block_2(x)
            x = self.block_3(x)
            flat_dim = x.view(x.shape[0], -1).shape[1]
        self.embedding = nn.Linear(flat_dim, self.d_model).to(device)

    def forward(self, x):
        if self.embedding is None:
            self.build_embedding(x.shape, x.device)
        x = self.block_1(x)
        x = self.block_2(x)
        x = self.block_3(x)
        x = x.view(x.shape[0], -1)

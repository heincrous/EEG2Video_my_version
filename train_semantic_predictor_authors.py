...
TEXT_PATH_ROOT = "/content/drive/MyDrive/EEG2Video_data/processed/CLIP_embeddings/blocks"
...
if __name__ == "__main__":
    ...
    print("EEG:", eegdata.shape)

    # Load text embeddings (aligned with GT_label already)
    text_blocks = []
    for block in range(6):
        path = os.path.join(TEXT_PATH_ROOT, f"block{block}.npy")
        text_blocks.append(np.load(path))
    text = np.stack(text_blocks, axis=0)

    # Reshape EEG and text
    eeg = rearrange(torch.from_numpy(eegdata[:6]), 'a b c d e -> (a b c) (d e)').numpy()
    text = rearrange(torch.from_numpy(text), 'a b c d e -> (a b c) (d e)').numpy()
    ...
    # Normalize EEG (authors' method = full normalization)

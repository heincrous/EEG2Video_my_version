...
class clip_score:
    ...
    def __init__(self, 
        ...
        self.device = device
        self.clip_model = CLIPVisionModelWithProjection.from_pretrained('Zhoutianyi/huggingface/clip-vit-large-patch14').to(device, torch.float16)
        self.clip_processor = AutoProcessor.from_pretrained('Zhoutianyi/huggingface/clip-vit-large-patch14')
...
def img_classify_metric(
    ...
    assert n_way > top_k
    processor = ViTImageProcessor.from_pretrained('Zhoutianyi/huggingface/vit-base-patch16-224')
    model = ViTForImageClassification.from_pretrained('Zhoutianyi/huggingface/vit-base-patch16-224').to(device, torch.float16)
    model.eval()
...
def video_classify_metric(
    ...
    assert n_way > top_k
    processor = VideoMAEImageProcessor.from_pretrained('Zhoutianyi/huggingface/videomae-base-finetuned-kinetics')
    model = VideoMAEForVideoClassification.from_pretrained('Zhoutianyi/huggingface/videomae-base-finetuned-kinetics', num_frames=num_frames).to(device, torch.float16)
    model.eval()
...
gt_path ='SEED-DV/video_16_9_512/block7/'

# ==========================================
# EEG → CLIP Semantic Predictor (DE-Only, MSE Loss, Optimisation-Tuning)
# ==========================================
# Same behaviour and scaling as the reference script:
# StandardScaler is fitted on ALL EEG data before splitting.
# Adds toggles for optimizer / scheduler / learning-rate experiments.
# Saves only final metrics and configuration (no checkpoints or outputs).
# ==========================================
...
# ==========================================
# Data Preparation (uses full-data fit for scaler)
# ==========================================
def prepare_data(eeg, clip, cfg):
    ...
    train_eeg_flat, val_eeg_flat, test_eeg_flat = map(flatten_eeg, [train_eeg, val_eeg, test_eeg])
    train_clip_flat, val_clip_flat, test_clip_flat = map(flatten_clip, [train_clip, val_clip, test_clip])

    # --- Fit scaler on ALL EEG (matches your reference script) ---
...
def train_model(model, train_loader, val_eeg, val_clip, cfg):
    ...
    for epoch in tqdm(range(1, cfg["num_epochs"] + 1)):
        ...
        for eeg, clip in train_loader:
            eeg, clip = eeg.to(device).float(), clip.to(device).float()
            optimizer.zero_grad()
        ...
        if epoch % 10 == 0:
            print(f"[Epoch {epoch:03d}/{cfg['num_epochs']}] Avg Loss: {total_loss/len(train_loader):.6f}")
...
def main(cfg):
    ...
    dataset = list(zip(torch.tensor(train_eeg), torch.tensor(train_clip)))

...
class TuneAVideoPipeline(DiffusionPipeline):
    ...
    def _encode_embeddings(self, embeddings, device, num_videos, do_cfg, negative_embeds=None):
        ...
        Handles unconditional (negative) embeddings if do_cfg=True.
        """
        bs, seq_len, _ = embeddings.shape
        embeddings = embeddings.to(device).to(torch.float16)
        embeddings = embeddings.repeat(1, num_videos, 1)
        embeddings = embeddings.view(bs * num_videos, seq_len, -1)

        if do_cfg:
            if negative_embeds is None:
                raise ValueError("negative_embeds must be provided when do_classifier_free_guidance=True")
            uncond = negative_embeds.to(device).to(torch.float16)
            uncond = uncond.repeat(1, num_videos, 1)
            uncond = uncond.view(bs * num_videos, seq_len, -1)
            embeddings = torch.cat([uncond, embeddings], dim=0)
    ...
    def prepare_latents(
        ...
        if isinstance(generator, list) and len(generator) != batch_size:
            raise ValueError(
                f"You passed {len(generator)} generators, but requested a batch size of {batch_size}. "
    ...
    def __call__(
        ...
        num_inference_steps: int = 50,
        guidance_scale: float = 7.5,
        negative_embeds: Optional[torch.FloatTensor] = None,
        ...
        **kwargs,
        ...
        # embeddings are already CLIP outputs, shape (B,77,768)
        text_embeddings = self._encode_embeddings(
            embeddings, device, num_videos_per_prompt, do_cfg, negative_embeds
        )

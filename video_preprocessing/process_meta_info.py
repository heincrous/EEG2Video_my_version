...
# Load all meta arrays into a dict
meta_data = {}
for fname in META_FILES:
    path = os.path.join(RAW_META_DIR, fname)
    meta_data[fname.split(".")[0]] = np.load(path)

# Expect shape (1400, ...) = 7 blocks * 40 classes * 5 clips
...
# Iterate over all 1400 clips
for idx in range(total_clips):
    block_id = idx // 200 + 1          # 200 clips per block
    class_id = (idx % 200) // 5 + 1    # 5 clips per class
    clip_id = (idx % 5) + 1

...
# ==========================================
# Full Inference (EEG → Video, using one precomputed CLIP embedding)
# ==========================================
import os, gc, torch, numpy as np
from diffusers import AutoencoderKL, DDIMScheduler
from transformers import CLIPTokenizer, CLIPTextModel
...
# ==========================================
PRETRAINED_SD_PATH = "/content/drive/MyDrive/EEG2Video_checkpoints/stable-diffusion-v1-4"
FINETUNED_SD_PATH  = "/content/drive/MyDrive/EEG2Video_checkpoints/diffusion_checkpoints/pipeline_final_subset0-2-4-10-11-12-22-26-29-37_20_epochs"
OUTPUT_DIR         = "/content/drive/MyDrive/EEG2Video_outputs/test_full_inference"
BLIP_TEXT_PATH     = "/content/drive/MyDrive/EEG2Video_data/processed/BLIP_text/BLIP_text.npy"
CLIP_EMB_PATH      = "/content/drive/MyDrive/EEG2Video_data/processed/CLIP_embeddings/CLIP_embeddings.npy"  # shape (7,40,5,77,768)
...
# ==========================================
# Run inference with the selected embedding
# ==========================================
def run_inference():
    fps = 3  # 2 seconds
    video = pipe(
        prompt=clip_embeddings,                   # precomputed [1,77,768]
        negative_prompt=empty_emb,                # unconditional [1,77,768]
        video_length=6,
        height=288,
        width=512,
        num_inference_steps=100,
    ...
    with open(os.path.join(OUTPUT_DIR, f"class{class_id}_trial{trial+1}.txt"), "w") as f:
        f.write(caption + "\n")

...
for block_id in range(7):
    ...
    for order_idx in tqdm(range(40), desc=f"Block {block_id+1}"):
        true_class = GT_LABEL[block_id, order_idx]  # map randomized → class
        for clip_id in range(5):
            flat_idx = order_idx * 5 + clip_id  # position inside block file
            caption = captions[flat_idx]

            # save text
            text_path = os.path.join(text_block_dir, f"class{true_class:02d}_clip{clip_id+1:02d}.txt")
            ...
            embed_path = os.path.join(embed_block_dir, f"class{true_class:02d}_clip{clip_id+1:02d}.npy")

...
# ==========================================
# ALIGN BLIP CAPTIONS TO CLIPS (with verification)
# ==========================================
# Input:
#   EEG2Video_data/raw/BLIP-caption/Xth_10min.txt
#   Each file contains 200 captions per block
...
# ==========================================
def verify_alignment(caption, embedding, tokenizer, encoder):
    with torch.no_grad():
        toks = tokenizer(
            caption, padding="max_length", truncation=True, max_length=77, return_tensors="pt"
        ).to(encoder.device)
        new_emb = encoder(**toks)[0].cpu().numpy()  # (1,77,768)
    mse = np.mean((new_emb[0] - embedding) ** 2)
    cos = np.sum(new_emb[0] * embedding) / (
        np.linalg.norm(new_emb[0]) * np.linalg.norm(embedding)
    )
    return mse, cos
...
for block_id in range(7):
    ...
    for start in tqdm(range(0, len(captions), config["batch_size"]), desc=f"Block {block_id+1}"):
        ...
        for j, caption in enumerate(batch_caps):
            ...
            # Verify only the very first caption in each block
            if flat_idx == 0:
                mse, cos = verify_alignment(caption, emb[j], tokenizer, encoder)
                print(f"[Block {block_id+1}] First caption verification → MSE={mse:.6f}, cos={cos:.6f}")
...
# ==========================================
# Global random verification
# ==========================================
b, c, t = random.randint(0,6), random.randint(0,39), random.randint(0,4)
cap = all_texts[b,c,t]
emb = all_embs[b,c,t]
mse, cos = verify_alignment(cap, emb, tokenizer, encoder)
print(f"\nRandom check [block {b}, class {c}, trial {t}] caption='{cap}'")
print(f"Verification → MSE={mse:.6f}, cos={cos:.6f}")
